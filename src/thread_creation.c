...
/*   Updated: 2022/12/19 09:35:44 by jmorneau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/philo.h"

void	supervisor(t_global *data)
{
	time_t	live;
	int		neat;
	int		i;
...
{	
	...
	{
		action(philo, time, THINK, 0);
		time_eat = get_time_in_ms();
		philo->last_meal = time_eat;
		action(philo, time, EAT, philo->time.eat_t);
		...
		action(philo, time, SLEEP, philo->time.sleep_t);
	}
	return (NULL);
}
...
int	thread_init(char **argv, t_global *data)
{
	...
	while (i < data->count)
	{
		if (pthread_create(&data->philos_thread[i],
				NULL, print_hello, &data->philos[i]))
			return (printf("Error\n"));
		if (data->time_delay)
			usleep(10);
		i++;
